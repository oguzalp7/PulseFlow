...
import { Tabs, TabList, TabPanels, Tab, TabPanel, Flex, Text, useToast, Spinner, SkeletonText, Skeleton, Box, Select, HStack, VStack, IconButton } from '@chakra-ui/react'
import DeviceCreateForm from '@/forms/device-create.form';
import CardGrid from '@/components/card-grid.component';
import DeviceCard from '@/components/device-card.component';
    ...
    return (
      <Tabs variant='soft-rounded' align='center' colorScheme='purple'  w={['sm', 'md', 'full']}  p={4} boxShadow="lg">
        <TabList border={'1px'}  borderRadius={'10px'} borderColor={'rgba(0, 255, 0, 0.3)'} mr={[10, 0]} paddingRight={[10, 0]} p={2} mb={4}  boxSize={['100%', '100%']} overflowX={'auto'}>
          {user && user.auth_id > 3 && <Tab color={'green'}>{language === 'en' ? 'New Device' : 'Yeni Cihaz'}</Tab>}
          <Tab color={'green'}>{language === 'en' ? 'Devices' : 'Cihazlarım'}</Tab>
